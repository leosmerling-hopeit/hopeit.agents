...
from hopeit_agents.mcp_server.server.mcp import run_app


@click.group()
def server() -> None:
    """CLI entry point for MCP server-related commands."""
...
@click.option(
    "--enabled-groups",
    default="",
    help="Optional comma-separated group labels to start. If no group is specified,"
    " all events will be"
    " started. Events with no group or 'DEFAULT' group label will always be started."
    " 'DEFAULT' group label"
...
@click.option(
    ...
    "restarted. Value is a positive number or 0. Set it to 0 for no timeout.",
)
def run(
    config_files: str,
    # api_file: str,
